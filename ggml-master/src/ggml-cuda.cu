...
static void ggml_cuda_op_mul_mat(
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        ggml_cuda_set_device(id);
        cudaStream_t stream =cudaStreamDefault;// ctx.stream(id, 0);
    ...
    for (int64_t src1_col_0 = 0; src1_col_0 < ne11; src1_col_0 += src1_col_stride) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            ggml_cuda_set_device(id);
            cudaStream_t stream = cudaStreamDefault;//ctx.stream(id, is);
...
static void ggml_cuda_op_mul_mat_1(
    ggml_backend_cuda_context & ctx,
    const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,char* workspace, ggml_cuda_op_mul_mat_t op,
    quantize_cuda_t quantize_src1) {
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        if (quantize_src1) {
            ...
            dev[id].src1_ddq = workspace;//dev[id].src1_ddq_alloc.alloc(ctx.pool(id), src_1_ddq_size);
...
void ggml_cuda_mul_mat_1(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ggml_backend_cuda_context * cuda_ctx = (ggml_backend_cuda_context *)backend->context;
    ggml_cuda_set_device(cuda_ctx->device);
    ggml_cuda_mul_mat(*cuda_ctx,src0,src1,dst);
    // ggml_cuda_op_mul_mat_1(*cuda_ctx,src0,src1,dst,workspace, ggml_cuda_op_mul_mat_vec_q, quantize_row_q8_1_cuda);
}
